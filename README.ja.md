...
キャッシュ可能なステップ(Chrome for Testing バイナリ、npm 依存、61MB ある Noto CJK フォント)はすべて自動でキャッシュされる。設定は不要。

| 実行 | `chromagic` ステップの所要時間 |
|---|---|
| コールド(初回・キャッシュ無し) | 約31〜42秒 |
| **ウォーム(キャッシュヒット、通常のPR実行)** | **約16秒** |

最小構成の Storybook サンプルリポジトリ、`ubuntu-latest`、2 vCPU で計測。
